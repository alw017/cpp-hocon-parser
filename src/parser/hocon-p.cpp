...
void HParser::insertHistory(std::vector<std::pair<std::vector<std::string>, std::variant<HTree*,HArray*,HSimpleValue*,HSubstitution*>>> list, size_t counter) {
    auto iter = stack.begin() + counter + 1;
    // update any counters after insertion

    // updating stack counters
    size_t listSize = list.size();
    if (listSize + counter + 1 < stack.size()) { // catches the edge case where a substitution is the last line in a .conf file.
        for (auto postInsertIter = stack.begin() + counter + listSize + 1; postInsertIter != stack.end(); postInsertIter++) {
    ...
    while(!atEnd()) { // loop through members
        ...
        if (match(LEFT_BRACE)) { // implied separator case. ex: foo {}
            ...
            if(check(SUB) || check(SUB_OPTIONAL)) {
                HSubstitution* sub;
                sub = (obj != nullptr) ? parseSubstitution(obj, rootPath, true) : parseSubstitution(rootPath, true);
...
HTree * HParser::hoconArraySubTree() { 
    ...
    while(!match(RIGHT_BRACE)) { // loop through members
        ...
        if (isInclude(peek())) {
            HTree * includedTree = parseInclude(std::vector<std::string>{"\"\""});
            consumeToNextMember();
            match(RIGHT_BRACE);
            delete output;
            return includedTree;
        ...
        if (match(LEFT_BRACE)) {            // implied separator case. ex: foo {}
            ...
            if(check(SUB) || check(SUB_OPTIONAL)) {
                HSubstitution* sub;
                sub = (obj != nullptr) ? parseSubstitution(obj, std::vector<std::string>(), true) : parseSubstitution(std::vector<std::string>(), true);
...
HTree * HParser::parseInclude(std::vector<std::string> rootPath) {
    ...
    if (std::get<0>(out) == "") {
        ...
        if (content == "" && !std::get<2>(out)){
            return nullptr;
        } else if (content == "") {
            error(peek().line, "include file " + std::get<0>(out) + " could not be opened.");
            return nullptr;
        }
...
HSubstitution * HParser::parseSubstitution(std::variant<HTree*, HArray*, HSimpleValue*> prefix, std::vector<std::string> parentPath, bool addingToStack) {
    ...
    while(!match(NEWLINE) && !check(COMMA) && !atEnd()) {
        if(match(LEFT_BRACE)){
            ...
            if (addingToStack) {
                HTree * obj = hoconTree(parentPath);
                if (obj) values.push_back(obj);
            } else {
...
void HParser::parseTokens() {
    ...
    if (match(LEFT_BRACKET)) { // root array
        ...
        if(!std::visit(valueExists, rootObject)) {
            rootObject = new HTree();
        }
        ignoreAllWhitespace();
...
std::variant<HTree*, HArray*, HSimpleValue*, HSubstitution*> HParser::resolveSub(HSubstitution* sub, std::unordered_set<HSubstitution*>& set, std::unordered_set<HSubstitution*> history) {
    ...
    for (size_t i = 0; i < sub->values.size(); i++) {
        ...
        if (std::holds_alternative<HPath*>(value)) {
            ...
            convertToStack(concatValue, list, absolutePath);
            // update any counters after insertion

            // updating stack counters
            size_t listSize = list.size();
            if (path->counter < stack.size()) { // catches the edge case where a substitution is the last line in a .conf file.
                for (auto postInsertIter = stack.begin() + path->counter; postInsertIter != stack.end(); postInsertIter++) {
                    if(std::holds_alternative<HSubstitution*>(postInsertIter->second)) {
                        HSubstitution * curr = std::get<HSubstitution*>(postInsertIter->second);
                        for (auto path : curr->paths) {
                            path->counter += listSize;
